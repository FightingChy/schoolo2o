...
import com.fighting.schoolo2o.dao.ProductImgDao;
...
import com.fighting.schoolo2o.util.PageCalculator;
...
public class ProductServiceImpl implements ProductService {
	...
	public ProductExecution getProductList(Product productCondition, int pageIndex, int pageSize) {
		...
		result.setCount(productList.size());
		return result;
	}
	...
	@Transactional
	//1.若缩略图参数有值，则处理缩略图
	//若原来存在缩略图则先山川再添加新图，之后获取缩略图相对路径并赋值给product
	//2.将商品详情图列表参数有值，对商品详情图片列表进行同样的操作
	...
	public ProductExecution modifyProduct(Product product, ImageHolder thumbnail, List<ImageHolder> productImgList)
		...
		if(product != null && product.getShop() != null && product.getShop().getShopId() != null) {
			...
			try {
				...
				if(effectedNum <= 0) {
					throw new ProductOperationException("创建商品图片失败");
				}
				return new ProductExecution(ProductStateEnum.SUCCESS, product);
			}catch (Exception e) {
				throw new ProductOperationException("创建商品失败:" + e.toString());
